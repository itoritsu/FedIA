...
class FedDBSCAN(FederatedModel):
    NAME = 'feddbscan'
    COMPATIBILITY = ['homogeneity']

    def __init__(self, nets_list, args, transform):
        super(FedDBSCAN, self).__init__(nets_list, args, transform)
        self.global_protos = []
        self.local_protos = {}
    ...
    def proto_aggregation(self,local_protos_list):
        ...
        for [label, proto_list] in agg_protos_label.items():
            if len(proto_list) > 1:
                ...
                cluster = DBSCAN(eps=self.args.eps, min_samples=self.args.min_samples, metric='cosine')
                c = cluster.fit_predict(proto_list)
                ...
                for index in range(c.shape[0]):
                    if c[index]==-1:
                        class_cluster_list.append(max(c[index])+1)
                    else:
                        class_cluster_list.append(c[index])
