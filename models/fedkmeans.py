...
class FedKmeans(FederatedModel):
    NAME = 'fedkmeans'
    COMPATIBILITY = ['homogeneity']

    def __init__(self, nets_list, args, transform):
        super(FedKmeans, self).__init__(nets_list, args, transform)
        self.global_protos = []
        self.local_protos = {}
    ...
    def proto_aggregation(self,local_protos_list):
        ...
        for [label, proto_list] in agg_protos_label.items():
            if len(proto_list) > 1:
                ...
                k_means = KMeans(n_clusters=self.args.n_clusters)

                c = k_means.fit(proto_list).labels_
