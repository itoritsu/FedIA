...
class FedProcg(FederatedModel):
    NAME = 'fedprocg'
    COMPATIBILITY = ['homogeneity']

    def __init__(self, nets_list, args, transform):
        super(FedProcg, self).__init__(nets_list, args, transform)
        self.global_protos = []
        self.local_protos = {}
    ...
    def _train_net(self, index, net, train_loader):
        ...
        for iter in iterator:
            ...
            f = feat[train_loader.train_mask]
            if len(self.global_protos) == 0:
                ...
                for label in train_loader.y[train_loader.train_mask]:
                    if label.item() in self.global_protos.keys():
                        ...
                        all_f_tensor = torch.cat(all_f, dim=0).to(self.device)  
                        keys = torch.tensor(all_global_protos_keys, device=self.device)  
                        i_pos = (keys == label.item()).nonzero(as_tuple=True)[0]  
                        f_pos = all_f_tensor[i_pos]  
                        i_neg = (keys != label.item()).nonzero(as_tuple=True)[0]
                        f_neg = all_f_tensor[i_neg]
            ...
            if iter == self.local_epoch-1:
                for i in range(len(train_loader.y[train_loader.train_mask])):
                    if train_loader.y[train_loader.train_mask][i].item() in agg_protos_label:
                        agg_protos_label[train_loader.y[train_loader.train_mask][i].item()].append(feat[train_loader.train_mask][i,:])
                    else:
                        agg_protos_label[train_loader.y[train_loader.train_mask][i].item()] = [feat[train_loader.train_mask][i,:]]
