...
class fggp(FederatedModel):
    ...
    def proto_aggregation(self, local_protos_list):
        ...
        for num, each_class_proto in agg_protos_label.items():
            ...
            y_hat = torch.full((proto.size(0),), num, device=self.device)
    ...
    def gradient_aggregation(self):
        """Aggregate gradients using the parent class logic and log results.
        Parameters containing ``self.args.personal`` will be left untouched."""

        personal_key = getattr(self.args, 'personal', None)

        
        saved_client_params = []
        if personal_key is not None:
            for net in self.nets_list:
                layer_weights = {name: param.data.clone()
                                 for name, param in net.named_parameters()
                                 if personal_key in name}
                saved_client_params.append(layer_weights)
            saved_global_params = {name: param.data.clone()
        ...
        if personal_key is not None:
            for net, layer_weights in zip(self.nets_list, saved_client_params):
                for name, weight in layer_weights.items():
                    for n, param in net.named_parameters():
                        if n == name:
                            param.data.copy_(weight)
                            break
            for name, weight in saved_global_params.items():
                for n, param in self.global_net.named_parameters():
                    if n == name:
                        param.data.copy_(weight)
                        break
